...
/**
 * POST /api/tenants/enterprise-invite
 *
 * Super-admin only. Creates a blank Enterprise/comp tenant and sends an
 * invitation email to the practitioner. Optional notes are logged only.
 *
 * Input:  { email, firstName, lastName, cabinetName?, locale?, notes? }
...
export async function POST(request: NextRequest) {
  ...
  const slug = `ent-${safeLastName.toLowerCase().replace(/[^a-z0-9]/g, '')}-${Date.now().toString(36)}`.slice(0, 40)
