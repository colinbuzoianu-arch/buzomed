...
export async function POST(request: NextRequest, ctx: RouteContext) {
  ...
  const previousSignatureUrl = target.signatureImageUrl ?? null
  ...
  if (!file || !(file instanceof File)) {
    return NextResponse.json({ error: 'Fișierul semnăturii lipsește.' }, { status: 400 })
  }
  ...
  const path = `${auth.user.tenantId}/signatures/${userId}-${Date.now()}.${ext}`
  ...
  if (uploadError) {
    console.error('[settings/signature] Upload failed:', uploadError)
  ...
  const oldPath = previousSignatureUrl?.split(`/${BUCKET}/`)[1]
  if (oldPath) {
    await supabase.storage.from(BUCKET).remove([oldPath])
  }

  return NextResponse.json({ signatureImageUrl })
}
...
export async function DELETE(request: NextRequest, ctx: RouteContext) {
  ...
  await prisma.user.update({ where: { id: userId }, data: { signatureImageUrl: null } })

  const oldPath = target?.signatureImageUrl?.split(`/${BUCKET}/`)[1]
  if (oldPath) {
    const supabase = createServiceClient()
    await supabase.storage.from(BUCKET).remove([oldPath])
  }
