...
const S = StyleSheet.create({
  page:         { fontFamily: 'Helvetica', fontSize: 9, paddingTop: 40, paddingBottom: 60, paddingHorizontal: 40, color: '#0f1e3f' },
  ...
  titleLabel:   { fontSize: 16, fontFamily: 'Helvetica-Bold' },
  ...
  partyLabel:   { fontSize: 7, fontFamily: 'Helvetica-Bold', color: '#64748b', letterSpacing: 0.8, marginBottom: 6 },
  ...
  tableHeadCell:{ fontSize: 7, fontFamily: 'Helvetica-Bold', color: '#64748b', letterSpacing: 0.5 },
  ...
  colTotal:     { width: 64, textAlign: 'right', fontFamily: 'Helvetica-Bold' },
  ...
  bankLabel:    { fontSize: 7, fontFamily: 'Helvetica-Bold', color: '#64748b', letterSpacing: 0.5, marginBottom: 4 },
  ...
  watermark:    { position: 'absolute', top: '40%', left: '15%', fontSize: 72, fontFamily: 'Helvetica-Bold', color: '#fee2e2', opacity: 0.5, transform: 'rotate(-35deg)', letterSpacing: 4 },
  ...
  return (
    <Document>
      <Page size="A4" style={S.page}>
        {isCancelled && <Text style={S.watermark}>ANULATA</Text>}
        ...
        <View style={S.header}>
          ...
          <View style={S.docMeta}>
            <Text style={{ fontSize: 7, color: '#94a3b8' }}>Factura platforma</Text>
        ...
        <View style={{ borderBottomWidth: 0.5, borderBottomColor: '#e2e8f0', paddingBottom: 14, marginBottom: 16 }}>
          <View style={S.titleRow}>
            <Text style={S.titleLabel}>FACTURA FISCALA</Text>
          ...
          <View style={S.metaRow}>
            ...
            {data.dueDate && (
              <View style={{ flexDirection: 'row', gap: 4 }}>
                <Text style={S.metaLabel}>Scadenta:</Text>
                <Text>{fmtDate(data.dueDate)}</Text>
              </View>
            )}
            {data.paidAt && (
              <View style={{ flexDirection: 'row', gap: 4 }}>
                <Text style={S.metaLabel}>Platita:</Text>
          ...
          <View style={S.totalsInner}>
            ...
            <View style={S.totalRow}>
              <Text style={S.totalLabel}>
                {isVatExempt ? 'TVA (scutit)' : `TVA ${(Number(data.vatRate) * 100).toFixed(0)}%`}
              </Text>
              <Text style={S.totalValue}>{fmt(data.vatAmount)} {data.currency}</Text>
            </View>
            <View style={S.grandRow}>
              <Text style={S.grandLabel}>Total de plata</Text>
          ...
          <View style={S.bankBlock}>
            ...
            {issuer.bank && (
              <View style={S.bankRow}>
                <Text style={S.bankKey}>Banca:</Text>
