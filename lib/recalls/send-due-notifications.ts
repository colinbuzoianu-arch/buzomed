...
export async function sendDueRecallNotifications(): Promise<{ sent: number; skipped: number }> {
  const now = new Date()
  ...
  for (const [, group] of byCompany) {
    ...
    if (result.success) {
      ...
      void logSystemError({
        route: 'recall-notifications',
        method: 'INTERNAL',
        error: new Error(result.error ?? 'send_failed'),
        context: { companyId: group.companyId, recallCount: group.recallIds.length },
      })
    }
  }
